...
interface TableShellHeadProps<T> {
  headers: TableShellHeader<T>[];
  sort: TableShellSortState | null;
  onSort: (columnIndex: number) => void;
}

function renderSortIcon(columnIndex: number, sort: TableShellSortState | null) {
...
function TableShellHead<T>({ headers, sort, onSort }: TableShellHeadProps<T>) {
  return (
